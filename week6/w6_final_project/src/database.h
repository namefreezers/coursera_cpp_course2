...
public:
    ...
    size_t RemoveIf(Pred pred) {
        ...
        for (map<Date, vector<string>>::iterator db_it = db.begin(); db_it != db.end(); ) {
            ...
            if (end_not_removed == events.begin()) {  // then remove entire vector
        ...
        for (pair<const Date, set<string>>& date_events_set : db_if_contains) { // todo
            const Date& date = date_events_set.first;
    ...
    template<typename Pred>
    vector<Entry<Date, string>> FindIf(Pred pred) const {
    ...
    Entry<Date, string> Last(const Date& d) const;
